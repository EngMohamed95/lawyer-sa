...
import { X, UserCheck, Mail, Phone, Lock, Save } from "lucide-react";
import { Button } from "./ui/button";

interface EditOfficeLawyerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  lawyerData: any;
...
export default function EditOfficeLawyerModal({ isOpen, onClose, onSuccess, lawyerData }: EditOfficeLawyerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lawyerRef = doc(db, "users", lawyerData.id);
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-[#133B2E] text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">تعديل بيانات المحامي</h2>
